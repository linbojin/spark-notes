...
*最后一节是动手编写自己的RDD API函数，介绍整个开发流程，如果你已经了解RDD模型，可以直接跳转到[编写RDD API函数](#编写rdd api函数)*
...
* **Fast**: Spark之前的Hadoop用的是MapReduce的编程模型，没有很好的利用分布式内存系统，中间结果都需要保存到external disk，运行效率很低。RDD模型是in-memory computing的，中间结果不需要被物化（materialized），它的**persistence**机制，可以保存中间结果重复使用，对需要迭代运算的机器学习应用和交互式数据挖掘应用，加速显著。Spark快还有一个原因是开头提到过的**Delay Scheduling**机制。
...
* partitioning：用户可以选择RDD元素被partitioned的方式来优化计算，比如两个需要被join的数据集可以用相同的方式做hash-partitioned
...
`withScope{body}`是为了确保运行body代码块产生的RDD在同一个scope里面。`val cleanF = sc.clean(f)`是为了确保这个f函数（专业称为closure）是可以被序列化的，因为它需要被序列化后发送到不同的node上。想要了解他们的实现细节可以用IDE代码跳转功能：
...
> 选择MapPartitionsRDD，command + b, 挑战到它的实现
...
MapPartitionsRDD[U, T]的partitioner和它的第一个parent RDD的partitioner保持一致如果需要保留partitioner，它的partitions就是它的firstParent的partitions。它的compute函数只是调用了flatMap实例化它时输入的函数。
再次聚焦需要传递的函数类型, 实际的函数和调用函数：
...
iter：Iterator[T]是一个Partition上的元素迭代器，用来遍历RDD[T]的第pid个partition上的所有元素。`firstParent[T].iterator(split, context)`就是用来准备parentRDD的对应partition的Iterator[T]的： 如果已经保存了就直接读取，否则重新计算。有了这个迭代器iter之后，然后用`iter.flatMap(cleanF)`来产生新的迭代器，返回类型是Iterator[U]，这个就是返回RDD[U]的partition的迭代器啦，`iter.flatMap(cleanF)`只是纯粹的scala flatMap计算，在不同的node上运行。
这部分内容，可能比较绕，只有看着源码不断的跳转，才能比较好的理解。总结一下，Transformations函数的实现，就是对RDD上每一个partition的iterator进行相应的操作。
...
## 编写RDD API函数
只说不练假把式，终于到了动手环节了，打起精神啦！任务是编写一个和scala的flatten函数类似的RDD函数:

```
scala> Seq(Seq(1,2), Seq(3)).flatten
res0: Seq[Int] = List(1, 2, 3)
```
虽然它可以用`flatMap(x => x)`来代替（这也是为什么spark RDD api函数里面没有它），但是自己动手实现一下这个简单的api函数，可以加深对RDD模型的理解，也能熟悉Spark开发流程。
